    ...
    <script type="text/javascript">
        ...
        function deleteStudent() {
            ...
            $.messager.confirm("系统提示", "您确认要删掉这<font color=red>" + selectedRows.length + "</font>条数据吗？", function (r) {
                if (r) {
                    $.post("${pageContext.request.contextPath}/stu?method=del",
                        {delIds: ids}, function (result) {
                            if (result.success) {
        ...
        function openStudentAddDialog() {
            ...
            url = "${pageContext.request.contextPath}/stu?method=save";
        }
        ...
        function openStudentModifyDialog() {
            ...
            if (selectedRows.length > 1) {
                $.messager.alert("系统提示", "不能修改多条记录！T_T");
                return;
            } else if (selectedRows.length == 0) {
                $.messager.alert("系统提示", "请选择记录T_T");
                return;
            }
            ...
            $("#fm").form("load", row);
            url =
                "${pageContext.request.contextPath}/stu?method=update&stuId=" +
                row.stuId;
        }
    </script>
</head>
...
<table id="dg" title="学生信息" class="easyui-datagrid" fitColumns="true"
       ...
       url="${pageContext.request.contextPath}/stu?method=stuList"
       fit="true"
       toolbar="#tb" loadMsg="请稍后～～╮(￣▽￣)╭">
    <thead>
    <tr>
...
<div id="tb">
    ...
    <div>&nbsp;学号：&nbsp;<input type="text" name="s_stuNo" id="s_stuNo"
                               size="10"/>
        &nbsp;姓名：&nbsp;<input type="text" name="s_stuName" id="s_stuName"
                              size="10"/>
        &nbsp;性别：&nbsp;<select class="easyui-combobox" id="s_sex" name="s_sex"
        ...
        &nbsp;出生日期：&nbsp;<input class="easyui-datebox" name="s_bbirthday"
                                id="s_bbirthday" editable="false"
                                size="10"/>-><input class="easyui-datebox"
                                                    name="s_ebirthday"
                                                    id="s_ebirthday"
                                                    editable="false" size="10"/>
        &nbsp;所属班级：&nbsp;<input class="easyui-combobox" id="s_gradeId"
                                name="s_gradeId" size="10"
                                data-options="panelHeight:'auto',editable:false,valueField:'gradeId',textField:'gradeName',url:'${pageContext.request.contextPath}/grade?method=gradeCbList'"/>
        ...
        <a href="javascript:searchStudent()" class="easyui-linkbutton"
           iconCls="icon-search" plain="true">搜索</a> <a
                ...
                iconCls="icon-reload" plain="true">重置</a></div>
</div>
</div>
<div id="dlg" class="easyui-dialog"
     style="width: 570px;height: 350px;padding: 10px 20px"
...
<div id="dlg-buttons">
    ...
    <a href="javascript:closeStudentDialog()" class="easyui-linkbutton"
